...
def Variance_Ratio_Test(a, n):
    ...
    for i in np.arange(0, len(a), n):
        ret_nth_period.append(np.sum(a[i:i + n-1]))
    print(ret_nth_period)
    var_nth_period = np.var(ret_nth_period)
    var_ratio = var_nth_period / (n * var_one_period)
    z_var_ratio = (var_ratio - 1) / np.sqrt((2 * (n - 1)) / (T * n))
...
def HS_Bootstrap_Full_Val_w_Opt(alpha, epsilon, n, B, port_weights, CVar_weights):
    ...
    for i in range(B):
        ...
        np.random.seed()  # placing a random seed
        ...
        # by dividing simulated prices by the security prices on 2018-02-09 respectively
        ...
        HS_full_B_list.append(sim_HS_full_Val)
        ...
        HS_full_ES_B_list.append(sim_HS_full_Val_ES)

    ## Comment out to see distribution of the bootstrap simulated VaR
    # (mu, std) = norm.fit(HS_full_B_list)
    # n, bins, patches = plt.hist(HS_full_B_list, 60, normed=1, facecolor='b')
    # y = mlab.normpdf(bins, mu, std)
    # l = plt.plot(bins, y, 'r--', linewidth=2)
    # plt.show()

    HS_full_Val = np.mean(HS_full_B_list)

    HS_full_Val_ES = np.mean(HS_full_ES_B_list)
    ...
    # Conditioning on portfolio returns lower than -VaR, return list of conditioned simulated securities returns
    for i in range(0, len(ten_day_sim_port_ret_HS)):
        if ten_day_sim_port_ret_HS[i] <= -HS_full_Val_VaR:
            m.append(ten_day_sim_sec_ret_HS[i])
    m = np.asarray(m)

    # the means of the conditioned securities returns are the MVaR of each security
    MVaR = -np.mean(l, axis = 0 )

    # the means of the conditioned securities returns are the MES of each security
    MES = -np.mean(m, axis=0)
    ...
    # multiply MVaR by percentage weights returns CVaR and CES
    CVaR = np.multiply(MVaR, percent_weights)
    CES = np.multiply(MES, percent_weights)

    # print out MVaR and CVaR of each security
    name = np.array(["VZ","INTC","JPM","AAPL","MSFT","PG","AAPL opt", "MSFT opt","PG opt"])
    MCVaRES = {'1. MVaR':MVaR, '2. CVaR':CVaR,'3. MES':MES, '4. CES':CES}

    print(pd.DataFrame(data=MCVaRES, index=name))       # TODO printing out MVaR and CVaR

    print(HS_full_Val_VaR, HS_full_Val_ES, (HS_full_Val_VaR - np.sum(CVaR)), (HS_full_Val_ES - np.sum(CES)), np.sum(CVaR), np.sum(CES))

    return HS_full_Val_VaR, HS_full_Val_ES, MVaR, CVaR, (HS_full_Val_VaR - np.sum(CVaR))
...
def HS_Bootstrap_Greek_App(alpha, epsilon_greeks, n, B, port_weights, CVaR_weights):
    ...
    for i in range(B):
        ...
        aapl_opt_greeks_val = call_int_AAPL \
                              + np.multiply(aapl_greeks[0], 10 / 360) \
                              + np.multiply(aapl_greeks[2],dS[:, 3]) \
                              + np.multiply(np.multiply(np.multiply(dS[:, 3], aapl_greeks[4]), dS[:, 3]), 0.5)
                              # + np.transpose(dS[:,3]).dot(np.diag(np.ones(B)*aapl_greeks[4]*0.5)).dot(dS[:,3])


        msft_opt_greeks_val = put_int_MSFT + np.multiply(msft_greeks[1], 10 / 360) \
                              + np.multiply(msft_greeks[3],dS[:, 4]) \
                              + np.multiply(np.multiply(np.multiply(dS[:, 4], msft_greeks[4]), dS[:, 4]), 0.5)
                              # + np.transpose(dS[:, 4]).dot(np.diag(np.ones(B) * msft_greeks[4] * 0.5)).dot(dS[:, 4])


        pg_opt_greeks_val = call_int_PG + np.multiply(pg_greeks[0], 10 / 360) \
                              + np.multiply(pg_greeks[2],dS[:, 5]) \
                              + np.multiply(np.multiply(np.multiply(dS[:, 5], pg_greeks[4]), dS[:, 3]), 0.5)
                              # + np.transpose(dS[:, 5]).dot(np.diag(np.ones(B) * pg_greeks[4] * 0.5)).dot(dS[:, 5])
    ...
    # Conditioning on portfolio returns lower than -VaR, return list of conditioned simulated securities returns
    for i in range(0, len(ten_day_sim_port_ret_HS_greeks)):
        if ten_day_sim_port_ret_HS_greeks[i] <= -HS_Val_VaR_greeks:
            m_greeks.append(ten_day_sim_sec_ret_HS_greeks[i])
    m_greeks = np.asarray(m_greeks)

    # Calculate MVaR by taking the mean of each conditioned securities return
    MVaR_greeks = -np.mean(l_greeks, axis=0)
    MES_greeks = -np.mean(m_greeks, axis=0)
    ...
    CES_greeks = np.multiply(MES_greeks, percent_weights_greeks)

    # print out MVaR and CVaR of each security
    name = np.array(["VZ", "INTC", "JPM", "AAPL", "MSFT", "PG", "AAPL opt", "MSFT opt", "PG opt"])
    MCVaRES = {'1. MVaR': MVaR_greeks, '2. CVaR': CVaR_greeks,'3. MES': MES_greeks, '4. CES': CES_greeks}

    print(HS_Val_VaR_greeks, HS_Val_greeks_ES)
    print(pd.DataFrame(data=MCVaRES, index=name))       # TODO printing out MVaR and CVaR
    ...
    for n in range(9):
        ...
        for i in np.arange(0,10,dw):
            ...
            if FullorGreeks == 1:
                ...
                [VaR, ES, MVaR, CVaR] = HS_Bootstrap_Greek_App(0.99, 0.005, 10, 10000, port_weights, CVaR_weights)
...
def main():
    ...
    ## pure guessing of epsilon for sum of CVaR summing up to VaR

    # for i in np.arange(0.0100, 0.05, 0.001):
    #     ans = HS_Bootstrap_Full_Val_w_Opt(0.99, i, 10, 500, port_weights, CVaR_weights)
    #     print(i)
    #     if abs(ans[4]) < 0.001:
    #         print(i)
    #         break
    HS_Bootstrap_Full_Val_w_Opt(0.99, 0.03, 10, 1000, port_weights, CVaR_weights)
